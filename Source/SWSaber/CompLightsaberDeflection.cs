...
{
    private const int DefenseMeleeBlockChance = 15;
    private readonly FloatRange reflectionReturnChance = new(0.15f, 0.25f);
    ...
    public override bool TrySpecialMeleeBlock()
    {
        ...
        var modifier = (int)AccessTools.Method(forceUser.GetType(), "ForceSkillLevel")
            .Invoke(forceUser, ["PJ_LightsaberDefense"]);
        var blockChance = 0;
        if (modifier <= 0)
        {
            return false;
    ...
    private int CalculatedAccuracy_ForceModifier()
    {
        ...
        {
            var modifier = (int?)AccessTools.Method(forceUser.GetType(), "ForceSkillLevel")
                .Invoke(forceUser, ["PJ_LightsaberReflection"]);
            if (modifier is null or <= 0)
            {
                return result;
            }
    ...
    //Placeholder for now.
    private static int CalculatedAccuracy_ForceDifficulty()
    {
        return 100;
    }

    //Placeholder for now.
    private static Verb CopyAndReturnNewVerb_ForceAdjustments(Verb newVerb)
    {
        return newVerb;
    }

    //Placeholder for now.
    private static Pawn ResolveDeflectionTarget_ForceAdjustments(Pawn defaultTarget = null)
    {
